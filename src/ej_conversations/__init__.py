...
def create_conversation(question, title, author, *, is_promoted=False, tags=(), commit=True):
    """
    ...
    conversation = _db.Conversation(question=question, title=title, author=author, status=status)
    conversation.clean()
    if commit:
        conversation.save()
    return conversation
