...
def scatter_pca_json(request, conversation, slug, check=check_promoted):
    ...
    return JsonResponse({
        'option': {
            'xAxis': {},  # {'show': False},
            'yAxis': {},  # {'show': False},
            'color': ['#042A46', '#FF3E72', '#30BFD3', '#36C273', '#7758B3', '#797979', '#F68128',
                      '#C4F2F4', '#B4FDD4', '#FFE1CA', '#E7DBFF', '#FFE3EA', '#EEEEEE'],
            'series': [{
                'symbolSize': 16,
                'data': data.tolist(),
                'type': 'scatter',
...
def scatter_group(request, conversation, slug, groupby, check=check_promoted):
    ...
    data_pairs = (User.objects
        .filter(votes__comment__conversation=conversation)
