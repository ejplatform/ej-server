...
from ej_users.manager import UserManager
...
class TestUser(TestCase):
    ...
    def test_get_profile_fields(self):
        ...
        fields = self.filled_user.profile_fields()
        for field in fields:
            assert True if field[1] in test_user_fields else False

    def test_get_profile_statistics(self):
        statistics = self.user.profile_statistics()
        self.assertTrue(statistics, {'votes': 0, 'comments': 0, 'conversations': 0})

    def test_get_role_description(self):
        self.assertTrue(self.user.role(), 'Regular user')

        self.user.is_staff = True
        self.assertTrue(self.user.role(), 'Administrative user')

        self.user.is_superuser = True
        self.assertTrue(self.user.role(), 'Root')
