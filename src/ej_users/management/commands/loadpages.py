...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--path", "-p", type=str, help="Path to look for pages")
        parser.add_argument(
            "--force", "-f", action="store_true", help="Override existing pages with file data."
        )
...
def real_handle(path, force=False):
    ...
    return [
        *[handle_html(x, y, kwargs) for x, y in html_files.items()],
        *[handle_markdown(x, y, kwargs) for x, y in md_files.items()],
    ]


def handle_html(path, file_name, kwargs):
    save_file(path, file_name, HTML_TITLE_RE, template="flatpages/html.html", **kwargs)


def handle_markdown(path, file_name, kwargs):
    save_file(path, file_name, MARKDOWN_TITLE_RE, template="flatpages/markdown.html", **kwargs)
...
def save_file(path, file_name, title_re, template, **kwargs):
    ...
    page, created = FlatPage.objects.update_or_create(
        url=file_name, defaults={"title": title, "content": data, "template_name": template}, **kwargs
    )
