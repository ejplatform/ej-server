...
def create_colors_experiment(skip_prob=0.25, n_times=1, n_users=21, has_yellow=False):
    ...
    from ej_clusters.models import Stereotype, Cluster, Clusterization, StereotypeVote
    ...
    conversation = create_conversation(
        'What is your favorite color?', 'Color', author, is_promoted=True)
    ...
    # Create comments
    comment_map = {
        color: repeat(n_times, comments, color, user_map[color][0], conversation)
        for color in rgb
    }
    ...
    votes = Vote.objects.bulk_create(votes)

    # Create personas and votes
    clusterization = Clusterization.objects.create(conversation=conversation, cluster_status=ClusterStatus.ACTIVE)
    stereotype_votes = []
    commit = True
    yellow_clusters = []
    for color in rgb:
