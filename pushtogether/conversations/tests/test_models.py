...
class TestConversation(TestCase):
    ...
    def setUp(self):
        self.user = create_valid_user("test_user")
        self.other_user = create_valid_user("other_user")
        self.conversation = create_valid_conversation(self.user)

    def test_create_valid_comment(self):
        old_counter = self.conversation.comments.count()
        create_valid_comment(self.conversation, self.user)
    ...
    def test_nudge_interval_comments(self):
        ...
        create_valid_comment(self.conversation, self.user)
        time.sleep(2)
        create_valid_comments(2, self.conversation, self.user)
    ...
    def test_nudge_is_user_eager_with_a_comment(self):
        ...
        create_valid_comment(self.conversation, self.user)
        user_comments = self.conversation._get_nudge_interval_comments(self.user)

        assert self.conversation._is_user_nudge_eager(
            user_comments.count(), user_comments) == True

    def test_nudge_is_user_eager_with_comment_nudge_minus_one(self):
        """
        Should return true if user is trying to post too much comments
        """
        self.conversation.comment_nudge = 6
        self.conversation.comment_nudge_interval = 2
        create_valid_comments(5, self.conversation, self.user)
        user_comments = self.conversation._get_nudge_interval_comments(self.user)
    ...
    def test_nudge_is_user_interval_blocked(self):
        ...
        create_valid_comment(self.conversation, self.user)
        user_comments_counter = self.conversation._get_nudge_interval_comments(self.user).count()
        is_user_blocked = self.conversation._is_user_nudge_interval_blocked(user_comments_counter)
    ...
    def test_nudge_is_user_global_limit_blocked(self):
        ...
        create_valid_comment(self.conversation, self.user)
        is_user_blocked = self.conversation._is_user_nudge_global_limit_blocked(self.user)
    ...
    def test_nudge_status_should_return_eager(self):
        ...
        """
        self.conversation.comment_nudge_global_limit = 30
        self.conversation.comment_nudge = 7
        self.conversation.comment_nudge_interval = 2
        create_valid_comments(6, self.conversation, self.user)
        nudge_status = self.conversation.get_nudge_status(self.user)
    ...
    def test_get_random_comment(self):
        ...
        """
        comments = create_valid_comments(3, self.conversation, self.user)
    ...
    def test_get_random_comment_should_return_only_approved_comments(self):
        ...
        """
        comments = [create_valid_comment(self.conversation, self.user, approval)
    ...
    def test_get_random_comment_should_return_only_unvoted_comments(self):
        ...
        """
        comment = create_valid_comment(self.conversation, self.user)
    ...
    def test_get_user_participation_ratio(self):
        ...
        """
        comment = create_valid_comment(self.conversation, self.other_user)
        create_valid_comment(self.conversation, self.user)
...
class TestVote(TestCase):
    def setUp(self):
        ...
        self.other_user = create_valid_user("other_user")
        self.conversation = create_valid_conversation(self.user)
        self.comment = create_valid_comment(self.conversation, self.user)
